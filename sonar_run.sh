...
PROJECTKEY="py-grade-stats-tool" 
Check=`curl -s -u admin:brutus2212 http://localhost:9000/api/qualitygates /project_status?projectKey=$PROJECTKEY | jq '.projectStatus.status' | tr - d '"'` 
max_retry=30 
counter=0 
...
QGSTATUS=`curl -s -u admin:brutus2212 http://localhost:9000/api/qualitygates /project_status?projectKey=$PROJECTKEY | jq '.projectStatus.status' | tr - d '"'` 
if [ "$QGSTATUS" = "OK" ] 
then 
echo "Status is OK" 
